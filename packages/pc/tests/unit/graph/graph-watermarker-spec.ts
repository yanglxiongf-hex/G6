...
describe('add addWatermark', () => {
  it.only('text watermark', () => {
    ...
    const waterMarkerCanvasDOM = children[1].children[0]
    expect(children[1].style.display).toEqual('none');
    expect(waterMarkerCanvasDOM.style.width).toEqual('150px');
    expect(waterMarkerCanvasDOM.style.height).toEqual('100px');
  ...
  it.only('image watermark', () => {
    ...
    expect(children[1].style.display).toEqual('none');
    expect(waterMarkerCanvasDOM.style.width).toEqual('150px');
    expect(waterMarkerCanvasDOM.style.height).toEqual('130px');
    graph.destroy();
  });

  it.only('text watermark width config param', () => {
    const graph = new Graph({
      container: div,
      width: 500,
      height: 500,
    ...
    expect(children[1].style.display).toEqual('none');
    const waterMarkerCanvasDOM = children[1].children[0]
    expect(waterMarkerCanvasDOM.style.width).toEqual('80px');
    expect(waterMarkerCanvasDOM.style.height).toEqual('60px');
